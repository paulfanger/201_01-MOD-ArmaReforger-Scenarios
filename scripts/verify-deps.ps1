...
#
# ASCII-only for PowerShell 5.1 default CP1252 encoding compatibility.
...
$wingetDeps = @(
    ...
    @{ id="AutoHotkey.AutoHotkey"; verify="where.exe AutoHotkey"; deprecated=$true }
)
...
foreach ($j in $junctions) {
    $leaf = Split-Path $j -Leaf
    if (Test-Path $j) {
        Write-Host "  [OK]      $leaf" -ForegroundColor Green
        $results.ok += "junction:$leaf"
    } else {
        Write-Host "  [MISSING] $leaf - run scripts/pc-setup.ps1" -ForegroundColor Red
        $results.missing += @{ kind="junction"; id=$leaf }
    }
}
...
foreach ($m in $results.missing) {
    if ($m.kind -eq "winget") {
        Write-Host ("    winget install --id " + $m.id + " -e --accept-package-agreements --accept-source-agreements")
    } elseif ($m.kind -eq "pip") {
        Write-Host ("    pip install --user " + $m.id)
    } elseif ($m.kind -eq "npm") {
        Write-Host ("    npm install -g " + $m.id)
    } elseif ($m.kind -eq "steam") {
        Write-Host ("    Install '" + $m.id + "' via Steam client manually")
    } elseif ($m.kind -eq "env") {
        Write-Host ("    Set env var " + $m.id + " - see MANUAL-GATES-CHECKLIST P3")
    } elseif ($m.kind -eq "junction") {
        Write-Host "    Run: powershell -ExecutionPolicy Bypass -File scripts\pc-setup.ps1"
    }
}
Write-Host ""

...
class AI_GeneratePlugin : WorldEditorPlugin
{
    ...
    override void Run()
    {
        ...
        string status = (errors.Count() == 0) ? "ok" : "partial";
    ...
    protected string Op_AttributeEdit(WorldEditorAPI api, SCR_AIOp op)
    {
        ...
        if (!ent)
        {
            // Fallback: search by class name prefix (e.g. "weatherManager" → "WeatherManagerEntity_0")
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (className.ToLower().Contains(op.target.ToLower()))
                {
                    ent = candidate;
                    break;
                }
    ...
    protected string Op_EntityCreate(WorldEditorAPI api, SCR_AIOp op)
    {
        ...
        string coords = op.coords.IsEmpty() ? "0 0 0" : op.coords;
    ...
    protected array<ref SCR_AIOp> ParseOps(string json)
    {
        ...
        int arrOpen = json.IndexOf("[", opsStart);
        if (arrOpen < 0)
            return result;
    ...
    protected SCR_AIOp ParseSingleOp(string obj)
    {
        ...
        op.layer_id = layerStr.IsEmpty() ? 0 : layerStr.ToInt();
        ...
        if (op.op_type == "batch")
        {
            ...
            if (subStart >= 0)
                op.sub_ops = ParseOps(obj.Substring(subStart));
        }
        return op;
    }
